...
class FetchDataController extends Controller
{
    ...
    /**
     * Fetch the total count of unpaid penalties.
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function fetchPendingOverduesCount()
    {
        try {
            LogFacade::info('Analytics: Fetching pending overdues count', [
                'user_id' => Auth::id(),
                'user_name' => Auth::user()->full_name ?? 'N/A',
                'timestamp' => now(),
            ]);

            $unpaidPenaltiesCount = Transaction::where('penalty_status', 'Unpaid')
                ->where(function ($query) {
                    $query->where('penalty_total', '>', 0)
            ...
            LogFacade::info('Analytics: Pending overdues count fetched successfully', [
                'unpaid_penalties_count' => $unpaidPenaltiesCount,
                'user_id' => Auth::id(),
                'timestamp' => now(),
            ]);

            return response()->json(['unpaid_penalties_count' => $unpaidPenaltiesCount]);
        } catch (\Throwable $e) {
            LogFacade::error('Analytics: Error fetching pending overdues count', [
                'error_message' => $e->getMessage(),
                'error_trace' => $e->getTraceAsString(),
                'user_id' => Auth::id(),
                'timestamp' => now(),
            ...
            return response()->json(['error' => 'Unable to fetch pending overdues count'], 500);
        }
    }
