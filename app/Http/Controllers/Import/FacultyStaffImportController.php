...
class FacultyStaffImportController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($activeImport) {
            ...
            if ($activeImport->type === 'employees' && $activeImport->initiated_by === Auth::id() && $activeImport->created_at->gt(now()->subMinutes(2))) {
        ...
        try {
            ...
            Log::error('Faculty/Staff Import: Failed to dispatch job', [
                'error_message' => $e->getMessage(),
                'user_id'       => Auth::id(),
            ]);
    ...
    public function status(Request $request, int $id)
    {
        ...
        if ($progress->isActive()) {
            ...
            if ($failedJob) {
                // Extract a human-readable error from the exception column
