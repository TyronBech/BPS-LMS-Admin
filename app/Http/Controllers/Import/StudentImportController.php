...
class StudentImportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!$request->has('page') && !$request->has('perPage')) {
            $oldFile = $request->session()->get('student_import_file');
            if ($oldFile) {
        ...
        return view('import.students.students', compact('showTable', 'activeImport'));
    }

    /**
     * Dispatches the student import to the queue.
     *
    ...
    public function store(Request $request)
    {
        ...
        $filePath = $request->session()->get('student_import_file');
    ...
    public function upload(Request $request)
    {
        ...
        try {
            if ($request->isMethod('post') && !$request->hasFile('file')) {
                ...
                $request->session()->put('student_import_edits_new', $sessionEditsNew);
                ...
                $request->session()->put('student_import_edits_existing', $sessionEditsExisting);
            } else if ($request->hasFile('file')) {
                // Storing new file
                $file = $request->file('file');
                $filePath = $file->store('temp_imports');
                $request->session()->put('student_import_file', $filePath);
                $request->session()->forget([
                    'student_import_edits_new',
                    'student_import_edits_existing',
                ]);
            ...
            $filePath = $request->session()->get('student_import_file');
            if (!$filePath || !\Illuminate\Support\Facades\Storage::exists($filePath)) {
                return redirect()->route('import.import-students')->with('toast-warning', 'Please select a file to upload.');
            }
    ...
    /**
     * Reads and parses student data from Excel, partitioned into new and existing.
     */
    ...
    /**
     * Returns the current progress of a student import job as JSON.
     */
