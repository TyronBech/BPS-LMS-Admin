...
class TransactionController extends Controller
{
    ...
    private function exportExcel(Collection $data, string $type, string $userType)
    {
        ...
        if ($userType === 'student') {
            ...
            // Employee summary
            $summarySheet->setCellValue('A10', 'Month');
            $summarySheet->setCellValue('B10', 'Total Books Borrowed');

            $summarySheet->getStyle("A10:B10")->getFont()->setBold(true)->setSize(10);
            ...
            $summarySheet->getStyle("A10:B{$currentRow}")->applyFromArray($styleArray);
        ...
        $detailedSheet->setCellValue('B10', 'Title');
        $detailedSheet->setCellValue('C10', 'Name');
        
        $roleHeader = ($userType === 'student') ? 'Grade & Section' : 'Position';
        $detailedSheet->setCellValue('D10', $roleHeader);
        ...
        if ($type && $type == 'Borrowed') {
            ...
            $detailedSheet->mergeCells('A8:I8');
            $detailedSheet->setCellValue('E10', 'Borrowed');
            $detailedSheet->setCellValue('F10', 'Due');
            $detailedSheet->setCellValue('G10', 'Returned');
            $detailedSheet->setCellValue('H10', 'Transaction Type');
            $detailedSheet->setCellValue('I10', 'Status');
        } else if ($type && $type == 'Reserved') {
            $detailedSheet->getColumnDimension('H')->setWidth(20);
            $detailedSheet->mergeCells('A7:H7');
            $detailedSheet->mergeCells('A8:H8');
            $detailedSheet->setCellValue('E10', 'Reserved');
            $detailedSheet->setCellValue('F10', 'Pickup Deadline');
            $detailedSheet->setCellValue('G10', 'Transaction Type');
            $detailedSheet->setCellValue('H10', 'Status');
        } else {
            $detailedSheet->getColumnDimension('H')->setWidth(20);
            $detailedSheet->getColumnDimension('I')->setWidth(20);
            ...
            $detailedSheet->mergeCells('A8:K8');
            $detailedSheet->setCellValue('E10', 'Reserved');
            $detailedSheet->setCellValue('F10', 'Pickup Deadline');
            $detailedSheet->setCellValue('G10', 'Borrowed');
            $detailedSheet->setCellValue('H10', 'Due');
            $detailedSheet->setCellValue('I10', 'Returned');
            $detailedSheet->setCellValue('J10', 'Transaction Type');
            $detailedSheet->setCellValue('K10', 'Status');
        }
