    ...
    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-md p-6">
      <form action="{{ route('import.upload-students') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div class="flex flex-col items-center gap-3 pt-2">
          ...
          <a href="{{ route('import.download-students-template') }}" download class="skip-loader text-xs text-secondary-600 hover:text-secondary-700 underline font-medium">
...
@if(isset($activeImport) && $activeImport && $activeImport->isActive())
  <x-import-progress-overlay
    ...
    :index-route="route('import.import-students')"
    import-label="Students"
  />
@else
  <x-import-progress-overlay
    status-url=""
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const btn = document.getElementById('btn-insert-students');
  if (!btn) return;
  ...
  btn.addEventListener('click', async function () {
    ...
    try {
      const response = await fetch('{{ route("import.store-students") }}', {
        method: 'POST',
        headers: {
      ...
      if (window.ImportOverlay._setStatusUrl) {
        window.ImportOverlay._setStatusUrl('{{ url("/admin/import/students/import-status") }}/' + data.progress_id);
      }
