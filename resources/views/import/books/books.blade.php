...
<div class="border-2 border-slate-700 rounded-lg flex flex-col text-center items-center mb-4 mt-2 w-full">
  <form action="{{ route('import.import-books') }}" method="POST" enctype="multipart/form-data">
    @csrf
    <label class="block mt-4 text-sm font-medium text-gray-600 for=" for="file_input">Upload a file in CSV or Excel format</label>
...
@else
<form action="{{ route('import.store-books') }}" method="POST" class="w-full">
  @csrf
...
@endif
@if(!$showTable)
<a href="{{ route('import.download-book-template') }}" class="skip-loader flex text-blue-600 hover:text-blue-800 dark:text-blue-200 dark:hover:text-blue-400 underline w-full ml-2">
