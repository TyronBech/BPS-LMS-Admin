...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
  ...
  <p class="mt-1 text-md/relaxed text-center">Import employee's data to be used in the application using the Excel format</p>
  ...
  @else
  <form id="import-form" action="{{ route('import.upload-faculties-staffs') }}" method="POST" class="w-full mt-6">
    @csrf
    <div class="overflow-x-auto">
      @include('import.employees.table')
    </div>
    <div class="flex justify-center">
      <button type="submit" formaction="{{ route('import.store-faculties-staffs') }}" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-2 focus:ring-green-300 font-medium rounded-lg text-md px-4 py-2 me-2 mb-2 mt-4">Insert to Database</button>
    </div>
  </form>
  @endif
  @if(!$showTable)
