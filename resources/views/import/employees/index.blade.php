    ...
    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-md p-6">
      <form action="{{ route('import.upload-faculties-staffs') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div class="flex flex-col items-center gap-3 pt-2">
          ...
          <a href="{{ route('import.download-employee-template') }}" download class="skip-loader text-xs text-secondary-600 hover:text-secondary-700 underline font-medium">
...
{{-- Progress overlay --}}
@if(isset($activeImport) && $activeImport && $activeImport->isActive() && $activeImport->type === 'employees')
