    ...
    <div class="md:flex md:justify-between">
      <div class="mb-10 md:mb-0 md:max-w-sm">
        <a href="{{ $settings->social_links['website'] }}" target="_blank" rel="noopener noreferrer" class="flex items-center skip-loader">
          <img src="{{ $settings->org_logo ? asset('storage/' . $settings->org_logo) : '' }}" class="h-16 me-3" alt="{{ $settings->org_name }} Logo" />
          <div>
            <span class="self-center text-xl font-semibold whitespace-nowrap dark:text-white">{{ $settings->org_name }}</span>
            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $settings->org_address }}</p>
          </div>
        </a>
      ...
      <div class="grid grid-cols-2 gap-8 sm:gap-6 sm:grid-cols-3 mx-5">
        ...
        <div>
          ...
          <ul class="text-gray-500 dark:text-gray-400 font-medium">
            <li class="mb-4">
              <a href="tel:{{ $settings->contact_number }}" class="hover:underline skip-loader">{{ $settings->contact_number }}</a>
            </li>
            <li class="mb-4">
              <a href="mailto:{{ $settings->email }}" class="hover:underline skip-loader">{{ $settings->email }}</a>
            </li>
            <li>
