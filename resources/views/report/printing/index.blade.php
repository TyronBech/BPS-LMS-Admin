...
<div class="container mx-auto px-4">
  <h1 class="text-3xl text-center font-bold text-gray-800 dark:text-white mt-8 mb-6">Printing & Photocopy Records</h1>
  <form action="{{ route('report.printing-search') }}" method="POST" class="auto-search-form">
    ...
    <div class="flex flex-col lg:flex-row lg:items-end lg:justify-center gap-3 mb-4">
      ...
      {{-- Printing Type Selector --}}
      <div class="flex flex-col w-full lg:w-auto lg:flex-1 lg:max-w-[150px]">
        <label for="printing_type" class="block text-xs font-medium mb-1 text-gray-500 dark:text-gray-400">Service</label>
  ...
  <div id="delete-printing-modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[100%] max-h-full bg-gray-900 bg-opacity-50">
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    deletePrintingBtn.forEach(btn => {
      btn.addEventListener('click', function(event) {
        ...
        if (deletePrintingID) {
          deletePrintingID.value = entryId;
        }
      });
    });
  });
