...
<div class="container mx-auto px-4">
  ...
  <div id="table-container">
    ...
    @include('report.non-circulations.table')

  </div>

  @can(PermissionsEnum::CREATE_NON_CIRCULATION_ENTRY)
  @include('report.non-circulations.form-modal')
  @endcan

  @can(PermissionsEnum::CREATE_NON_CIRCULATION_ENTRY->value, 'admin')
  <div id="delete-non-circulation-modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[100%] max-h-full bg-gray-900 bg-opacity-50">
    <div class="relative p-4 w-full max-w-md max-h-full">
      <div class="relative bg-white rounded-lg dark:bg-gray-700 shadow-md">
        <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="delete-non-circulation-modal">
        ...
        <div class="p-4 md:p-5 text-center">
          ...
          <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure you want to delete this non-circulation entry?</h3>
          <form action="{{ route('report.non-circulation-delete') }}" method="POST" class="flex items-center justify-center">
            @csrf
            @method('DELETE')
            <input type="hidden" name="id" id="delete_non_circulation_id" value="" />
            <button data-modal-hide="delete-non-circulation-modal" type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
              Yes, I'm sure
            </button>
            <button data-modal-hide="delete-non-circulation-modal" type="button" class="skip-loader py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-primary-500 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-50 dark:border-gray-600 dark:hover:text-primary-50 dark:hover:bg-gray-700 shadow-md">No, cancel</button>
          </form>
        </div>
      </div>
    </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    const deleteNonCirculationBtn = document.querySelectorAll('.deleteNonCirculationBtn');
    const deleteNonCirculationID = document.getElementById('delete_non_circulation_id');
    deleteNonCirculationBtn.forEach(btn => {
      btn.addEventListener('click', function(event) {
        const entryId = this.value || event.currentTarget.value;
        if (deleteNonCirculationID) {
          deleteNonCirculationID.value = entryId;
        }
      });
    });
  });
