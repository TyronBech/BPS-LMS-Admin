...
<div class="container mx-auto px-4 sm:px-6 lg:px-8">
  ...
  <div class="w-full p-6 bg-white border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700 shadow-md">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4">
      <div>
        <h5 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Add Video</h5>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Adding to folder: <strong>{{ $folder->name }}</strong></p>
      </div>
    ...
    <form action="{{ route('maintenance.library-website.gallery.store-video') }}" method="POST" class="max-w-2xl mx-auto">
      @csrf
      ...
      <div class="grid grid-cols-1 gap-6 mb-6">
        ...
        <div>
          ...
          <input type="text" id="title" name="title" value="{{ old('title') }}" required
            ...
            placeholder="e.g. Graduation Ceremony 2024 Highlights">
          @error('title')
      ...
      <div class="flex justify-end mt-6 gap-4">
        ...
        <button type="submit" class="text-white bg-primary-500 hover:bg-primary-400 focus:ring-4 focus:ring-primary-400 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-primary-400 dark:hover:bg-primary-500 focus:outline-none dark:focus:ring-primary-500">Add Video</button>
      </div>
    </form>
  </div>
</div>
