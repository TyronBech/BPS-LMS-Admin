      ...
      <div class="flex flex-col sm:flex-row items-center gap-4 mt-4 sm:mt-0">
        <form action="{{ route('maintenance.search-admin') }}" method="POST" class="flex items-center w-full sm:w-auto">
          ...
          <div class="relative w-full">
            ...
            <input type="text" id="search" name="search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" placeholder="Search admin" required />
          </div>
        ...
        <form action="{{ route('maintenance.create-admin', ['return_to' => request()->fullUrl()]) }}" method="GET" class="w-full sm:w-auto">
          <button type="submit" class="w-full text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">Add New Admin</button>
        </form>
      </div>
    </div>
