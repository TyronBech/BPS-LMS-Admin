    ...
    <table class="w-full text-sm text-left rtl:text-right whitespace-nowrap table-auto">
      ...
      <tbody>
        ...
        <tr class="bg-white border-b text-center dark:bg-gray-800 dark:border-gray-600">
          ...
          <td class="pb-1">{{ $item->lrn ? $item->lrn : '-' }}</td>
          ...
          <td class="pb-1">{{ $item->grade_level ? $item->grade_level : '-' }}</td>
          <td class="pb-1">{{ $item->section ? $item->section : '-' }}</td>
          <td class="pb-1">{{ $item->groups->group_name }}</td>
          <td class="pb-1 px-5">{{ $item->email }}</td>
          <td class="pb-1 px-5">{{ $item->employees->employee_id ? $item->employees->employee_id : '-'}}</td>
          <td class="pb-1 px-5">{{ $item->employees->school_org ? $item->employees->school_org : '-'}}</td>
