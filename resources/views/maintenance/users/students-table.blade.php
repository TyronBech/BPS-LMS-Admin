...
<div class="container mx-auto px-2 font-sans flex-col">
  <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 justify-between items-center mb-4 w-full">
    ...
    <form method="GET" class="justify-end m-2 w-full sm:w-auto">
      ...
      <input type="number" name="perStudentPage" id="perPage" min="1" max="500" value="{{ old('perStudentPage', $perStudentPage) }}" onchange="this.form.submit()" class="border border-gray-300 text-xs rounded-lg focus:ring-primary-400 focus:border-primary-400 p-2 my-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" />
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    document.addEventListener('click', function(event) {
      const deleteBtn = event.target.closest('.deleteStudentBtn');
      if (deleteBtn) {
        const deleteStudentID = document.getElementById('delete_student_id');
        if (deleteStudentID) {
          deleteStudentID.value = deleteBtn.value;
        }
      }
    });
