    ...
    <div class="w-full p-4 sm:p-6 bg-white border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700 shadow-md">
        ...
        <form action="{{ route('maintenance.update-book') }}" method="POST" enctype="multipart/form-data" class="mt-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                ...
                <div class="md:col-span-2">
                    <label for="subject_search" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Subjects:</label>
                    <div class="relative" id="multiselect-subject">
                        ...
                        <div id="subject-dropdown" class="absolute z-30 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-xl hidden max-h-60 overflow-y-auto">
                            <ul id="subject-options-list" class="py-1 text-sm text-gray-700 dark:text-gray-200"></ul>
                    ...
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400 italic">Click to select subjects. Search to filter. Configure in Subject Maintenance.</p>
                    @error('subject_ids')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex flex-col sm:flex-row justify-end mt-6 gap-2">
                <button type="submit" class="text-white bg-primary-500 hover:bg-primary-400 focus:ring-4 focus:outline-none focus:ring-primary-400 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-400 dark:hover:bg-primary-500 dark:focus:ring-primary-500">Update Book</button>
            ...
            <form action="{{ route('maintenance.copy-book') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="p-4 md:p-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            <label for="copy_subject_search" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Subjects:</label>
                            <div class="relative" id="copy-multiselect-subject">
                                <div id="copy-subject-tags-container" class="flex flex-wrap gap-2 p-2.5 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus-within:ring-primary-500 focus-within:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white min-h-[45px] cursor-text">
                                    <input type="text" id="copy_subject_search" class="flex-grow bg-transparent border-none focus:ring-0 p-0 text-sm min-w-[150px] placeholder-gray-400" placeholder="Search and select subjects...">
                                </div>
                                <div id="copy-subject-dropdown" class="absolute z-30 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-xl hidden max-h-60 overflow-y-auto">
                                    <ul id="copy-subject-options-list" class="py-1 text-sm text-gray-700 dark:text-gray-200"></ul>
                                    <div id="copy-no-subjects-found" class="px-4 py-2 text-gray-500 dark:text-gray-400 hidden">No subjects found.</div>
                                </div>
                                <select name="subject_ids[]" id="copy_subject_ids" class="hidden" multiple>
                                    @foreach($subjects as $subject)
                                    <option value="{{ $subject->id }}" {{ in_array($subject->id, old('subject_ids', $linkedSubjectIds)) ? 'selected' : '' }}>
                                        {{ $subject->name }}{{ $subject->ddc ? ' (DDC: '.$subject->ddc.')' : '' }}
                                    </option>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Generic multiselect tag handler
        function setupMultiselect(searchInputId, dropdownId, optionsListId, tagsContainerId, hiddenSelectId, noResultsId) {
            const subjectSearch = document.getElementById(searchInputId);
            const subjectDropdown = document.getElementById(dropdownId);
            const subjectOptionsList = document.getElementById(optionsListId);
            const subjectTagsContainer = document.getElementById(tagsContainerId);
            const subjectHiddenSelect = document.getElementById(hiddenSelectId);
            const noSubjectsFound = document.getElementById(noResultsId);
        ...
        setupMultiselect('subject_search', 'subject-dropdown', 'subject-options-list', 'subject-tags-container', 'subject_ids', 'no-subjects-found');
        setupMultiselect('copy_subject_search', 'copy-subject-dropdown', 'copy-subject-options-list', 'copy-subject-tags-container', 'copy_subject_ids', 'copy-no-subjects-found');
    });
</script>
@endsection
