...
<head>
  ...
  <title>503 • Library Closed for Maintenance</title>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .card{
      background: linear-gradient(180deg, rgba(255,255,255,.06), rgba(255,255,255,.03)), linear-gradient(180deg, var(--panel), var(--panel-2));
    ...
    /* Books (simple blocks, monochrome palette) */
    .books{ position:absolute; inset:0; }
    .book{
      position:absolute; bottom: calc(100% - 10px); /* sits on shelf */
      width:22px; height:80px; border-radius:3px 3px 2px 2px;
      background:
    ...
    .book::after{
      content:"";
      position:absolute; left:4px; right:4px; top:10px; height:2px;
      background: rgba(255,255,255,.18);
      box-shadow: 0 8px 0 rgba(255,255,255,.14), 0 16px 0 rgba(255,255,255,.1);
      opacity:.8;
    }
    /* Shade variants with only blue/white */
    .b1{ background: linear-gradient(180deg, rgba(255,255,255,.14), rgba(255,255,255,.03)), rgba(32,36,108,.92) }
    .b2{ background: linear-gradient(180deg, rgba(255,255,255,.10), rgba(255,255,255,.02)), rgba(32,36,108,.98) }
    .b3{ background: linear-gradient(180deg, rgba(255,255,255,.18), rgba(255,255,255,.04)), rgba(32,36,108,.88) }
    .b4{ background: linear-gradient(180deg, rgba(255,255,255,.09), rgba(255,255,255,.02)), rgba(32,36,108,1) }
    .b5{ background: linear-gradient(180deg, rgba(255,255,255,.2), rgba(255,255,255,.05)), rgba(32,36,108,.85) }
    ...
    .rope{
      position:absolute; left:50%; top:-38px; width:2px; height:42px;
    ...
    .sign small{
      ...
      color:rgba(32,36,108,.9); font-weight:600; letter-spacing:1px; font-size:12px;
    }
    @keyframes swing{
    ...
    .mote{
      position:absolute; width:3px; height:3px; background:rgba(255,255,255,.7); border-radius:50%;
      filter: blur(.3px);
      animation: drift linear infinite;
      opacity:.7;
    }
    .m1{ left:10%; top:70%; animation-duration:9s }
    .m2{ left:30%; top:50%; animation-duration:12s; width:2px; height:2px; opacity:.55 }
    .m3{ left:65%; top:60%; animation-duration:10s }
    .m4{ left:82%; top:40%; animation-duration:13s; width:2px; height:2px; opacity:.55 }
    .m5{ left:48%; top:30%; animation-duration:11s }
    @keyframes drift{
      0%{ transform: translateY(0) translateX(0); opacity:.15 }
      50%{ opacity:.7 }
      100%{ transform: translateY(-60px) translateX(10px); opacity:.15 }
    }

    footer{
      margin-top:10px; color:var(--muted); font-size:12px;
    }
    code.k{ color:var(--ink); background:rgba(255,255,255,.08); padding:2px 6px; border-radius:6px; border:1px solid rgba(255,255,255,.1) }
  </style>
</head>
<body>
  <main class="wrap" role="main" aria-labelledby="page-title">
    <section class="card">
      <div class="brand">
        ...
        <span>BPS Learning Management</span>
      </div>

      <h1 id="page-title">503 — Library Closed for Maintenance</h1>
      <p class="lede">
        Our stacks are being reorganized and dusted. Service is temporarily unavailable.
        Please check back shortly.
      </p>

      <div class="actions">
        <a class="btn primary" href="javascript:location.reload()" aria-label="Retry loading this page">Try again</a>
        <a class="btn" href="{{ url('/') }}">Go to homepage</a>
        <a class="btn" href="mailto:support@example.com?subject=503%20Service%20Unavailable">Contact support</a>
      </div>

      <p class="hint">
        Error code: <code class="k">503 Service Unavailable</code>
      </p>

      <footer aria-hidden="true">We’ll reopen the reading room as soon as maintenance is complete.</footer>
    </section>

    <section class="scene" aria-label="Bookshelves with a hanging closed sign">
      ...
      <div class="sign" aria-hidden="true">
        ...
        <div class="board">
          <h2>CLOSED</h2>
          <small>Maintenance in progress</small>
        </div>
      </div>
