...
<div class="w-full p-6 bg-white border border-gray-200 rounded-lg shadow  dark:bg-gray-900 dark:border-gray-600">
  ...
  <form action="{{ route('maintenance.roles-and-permissions.update-role') }}" class="max-w-2xl mx-auto" method="POST">
    ...
    <div class="mb-5">
      ...
      @else
      <input type="text" id="role" name="role" class="bg-gray-50 border border-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Admin" value="{{ $role->name }}">
      @endif
      @error('role')
