  ...
  <div class="flex flex-col rounded-lg bg-white dark:bg-gray-800 shadow-sm">
    ...
    <form id="inventory-form" action="{{ route('inventory.dashboard') }}" method="POST" class="flex flex-col">
      ...
      <div class="flex items-center m-2">
        ...
        <select name="perPage" id="perPage" onchange="submitInventoryForm()" class="border border-gray-300 text-xs rounded-lg focus:ring-primary-500 focus:border-primary-500 p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white">
      ...
      <button type="submit" formaction="{{ route('inventory.update') }}" formmethod="POST" class="text-white max-w-36 self-center bg-primary-500 hover:bg-primary-400 focus:ring-4 focus:ring-primary-400 font-medium rounded-lg text-sm px-5 py-2.5 my-4 dark:bg-primary-400 dark:hover:bg-primary-500 focus:outline-none dark:focus:ring-primary-500">Save</button>
      @endif
    </form>
  </div>
</div>
