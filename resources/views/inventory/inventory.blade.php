...
@if(session('scan-remark-warning'))
@php($scanWarning = session('scan-remark-warning'))
<div id="remark-warning-modal" tabindex="-1" class="hidden fixed left-0 right-0 top-0 z-50 h-[calc(100%-1rem)] w-full items-center justify-center overflow-y-auto overflow-x-hidden md:inset-0">
  <div class="relative max-h-full w-full max-w-md p-4">
    <div class="relative rounded-lg bg-white shadow-sm dark:bg-gray-700">
      <button type="button" class="absolute end-2.5 top-3 inline-flex h-8 w-8 items-center justify-center rounded-lg bg-transparent text-sm text-gray-400 hover:bg-gray-200 hover:text-gray-900 dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="remark-warning-modal">
      ...
      <div class="p-4 text-center md:p-5">
        ...
        <h3 class="mb-2 text-lg font-semibold text-gray-800 dark:text-gray-100">Material status warning</h3>
        <p class="mb-3 text-sm text-gray-500 dark:text-gray-300">
          This material is currently marked as <span class="font-semibold">{{ $scanWarning['remarks'] }}</span>.
        </p>
        <p class="mb-5 text-sm text-gray-500 dark:text-gray-300">
          Accession: <span class="font-semibold">{{ $scanWarning['accession'] }}</span><br>
          Title: <span class="font-semibold">{{ $scanWarning['title'] }}</span>
        </p>
        <button data-modal-hide="remark-warning-modal" type="button" class="inline-flex items-center rounded-lg bg-amber-500 px-5 py-2.5 text-sm font-medium text-white hover:bg-amber-600 focus:outline-none focus:ring-4 focus:ring-amber-300 dark:focus:ring-amber-800">
          Ok, continue
        </button>
      </div>
    </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    const modalEl = document.getElementById('remark-warning-modal');
    if (!modalEl) {
      return;
    }
    ...
    const hideButtons = modalEl.querySelectorAll('[data-modal-hide="remark-warning-modal"]');
