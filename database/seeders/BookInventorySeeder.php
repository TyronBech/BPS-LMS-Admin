...
class BookInventorySeeder extends Seeder
{
    ...
    {
        $this->addInventoryWithAvailableRemarks();
    }
    private function addInventoryWithMissingRemarks()
    {
        DB::beginTransaction();
        try {
    ...
    private function addInventoryWithAvailableRemarks()
    {
        DB::beginTransaction();
        try {
            Log::info('Seeding book inventory from existing books...');
            ...
            // Making all the books' remarks to "On Shelf"
            Log::info('Changing the remarks of all books into "On Shelf"');
            Book::query()->update(['remarks' => 'On Shelf']);
            DB::commit();
            Log::info('Book inventory seeding completed successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
